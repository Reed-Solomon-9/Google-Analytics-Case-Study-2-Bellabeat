...
--Check values in sleep by hour table
SELECT
	*
FROM SleepBellaBeatByHour

WHERE user_id = '1644430081'

--Confirm number of unique users who measured sleep
...
INNER JOIN (
	...
	SELECT
		user_id, time,
		MAX(heartrate) AS max_hourly_heartrate
...
DROP TABLE IF EXISTS HeartrateByHourBellaBeat2;
CREATE TABLE HeartrateByHourBellaBeat2 AS
WITH TimeAsString AS (
...
SELECT
	user_id,
	CONCAT(hour, ':00:00')::TIMESTAMP AS hour,
	COUNT(*)/60 AS num_minutes_recorded,
	AVG(heartrate) AS avg_heartrate,
	MIN(heartrate) AS min_heartrate,
	PERCENTILE_DISC(0.1) WITHIN GROUP (ORDER BY heartrate) AS decile1_heartrate,
	PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY heartrate) AS median_heartrate,
	...
	SUM(CASE WHEN heartrate > (all_time_max/2) THEN 1 ELSE 0 END) AS num_effort_seconds
...
ORDER BY user_id;
--check UNIONed hourly table with both months included, confirm number of unique users who measured heart rate
SELECT
	user_id,
	MAX(num_effort_seconds)

FROM HeartrateByHourBellaBeat2
	
GROUP BY user_id

ORDER BY user_id
--**Query for combined table**
DROP TABLE IF EXISTS CombinedTableBellaBeat;
CREATE TABLE CombinedTableBellaBeat AS (
SELECT
	t2.username,
	t1.user_id,
	t1.hour,
	...
	t7.max_heartrate,
	t7.num_minutes_recorded 	AS heartrate_minutes_tracked,
...
LEFT JOIN IntensityByHourBellaBeatNew AS t5 ON t5.user_id = t1.user_id AND t5.hour = t1.hour
LEFT JOIN StepsByHourBellaBeat AS t6 		ON t6.user_id = t1.user_id AND t6.hour = t1.hour
LEFT JOIN HeartrateByHourBellaBeat2 AS t7 	ON t7.user_id = t1.user_id AND t7.hour = t1.hour


ORDER BY t2.username, t1.hour
)

--pull an individual user's metrics
SELECT 
	username,
	calories,
	mets,
...
FROM CombinedTableBellaBeat

WHERE username = 'User 11'

--pull and individual user's metrics for heartrate
SELECT 
	username,
	avg_heartrate,
	decile1_heartrate,
	...
	max_heartrate,
	num_effort_seconds,
	heartrate_minutes_tracked

FROM CombinedTableBellaBeat

WHERE username = 'User 34'	
--determine when the latest entry in the table was
SELECT 
	MAX(hour)

FROM CombinedTableBellaBeat	
--Create daily activity table from hourly table
WITH DayTable AS (
SELECT
	username,
	hour::DATE,
	ROUND(SUM(sedentary_mins)/60, 2) AS sedentary_hrs,
	ROUND(SUM(light_mins)/60, 2) AS light_hrs,
	ROUND(SUM(moderate_mins)/60, 2) AS moderate_hrs,
	ROUND(SUM(intense_mins)/60, 2) AS intense_hrs,
	ROUND(SUM(total_sleep_mins)/60, 2) AS total_sleep_hrs

FROM CombinedTableBellaBeat

GROUP BY username, hour::DATE

ORDER BY hour::DATE, username

)

SELECT
	username,
	ROUND(AVG(sedentary_hrs), 2) AS avg_sedendary_hrs,
	ROUND(AVG(light_hrs), 2) AS avg_light_hrs,
	ROUND(AVG(moderate_hrs), 2) AS avg_moderate_hrs,
	ROUND(AVG(intense_hrs), 2) AS avg_intense_hrs,
	ROUND(AVG(total_sleep_hrs), 2) AS avg_sleep_hrs
...
SELECT
	...
	SUM(mets) AS mets,
	ROUND(AVG(avg_weight), 2) AS avg_weight,
	SUM(sleep_1_mins) AS sleep_1_minutes,
	SUM(sleep_2_mins) AS sleep_2_minutes,
	SUM(sleep_3_mins) AS sleep_3_minutes,
	SUM(total_sleep_mins) AS total_sleep_minutes,
	ROUND(AVG(avg_sleep_value), 2) AS avg_hourly_sleep_value,
	SUM(total_sleep_value) AS total_sleep_value,
	SUM(intense_mins) AS intense_mins,
	SUM(moderate_mins) AS moderate_mins,
	SUM(light_mins) AS light_mins,
	SUM(sedentary_mins) AS sedentary_mins,
	ROUND(AVG(total_intensity_hour), 2) AS avg_total_hourly_intensity,
	ROUND(AVG(avg_intensity_per_min), 3) AS avg_intensity_per_min,
	SUM(steps) AS steps,
	ROUND(AVG(avg_heartrate), 2) AS avg_hourly_heartrate,
	ROUND(AVG(min_heartrate), 2) AS avg_min_hourly_heartrate,
	ROUND(AVG(decile1_heartrate), 2) AS avg_decile1_hourly_heartrate,
	ROUND(AVG(median_heartrate), 2) AS avg_median_hourly_heartrate,
	ROUND(AVG(decile9_heartrate), 2) AS avg_decile9_hourly_heartrate,
	ROUND(AVG(max_heartrate), 2) AS avg_max_hourly_heartrate,	
	SUM(heartrate_minutes_tracked) AS heartrate_minutes_tracked,
	SUM(num_effort_seconds) AS num_effort_seconds
